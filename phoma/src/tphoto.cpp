...
TPhoto TPhoto::getFromQuery(QSqlQuery &query)
{
    ...
    photo.removed = query.value(5).toInt();
...
void TPhoto::save()
{
    ...
    if (photoExist) {
        query.prepare("UPDATE photos SET title = :title, description = :description, photo = :photo, owner_id = :owner_id, section_id = :section_id, removed = :removed WHERE id=:id");
        query.bindValue(":id", id);
